...
describe("BlockChunk", function(){
	...
	describe("#addBlock", function(){
		it("should be able to retrieve later", function(){
			var blockChunk = new BlockChunk();
			var block = new Block();
			blockChunk.addBlock(new BlockCoord(1, 2, 3), block);
			expect(blockChunk.getBlock(new BlockCoord(1, 2, 3))).to.exist;
		})

		it("should throw if out of bounds", function(){
			var blockChunk = new BlockChunk(new BlockCoord(1, 1, 1), 2);
			expect(function(){
				blockChunk.addBlock(new BlockCoord(0, 0, 0), new Block());
			}).to.throw("out of bounds");
			expect(function(){
				blockChunk.addBlock(new BlockCoord(3, 3, 3), new Block());
			}).to.throw("out of bounds");
		})

		it("shoudn't throw if no bounds defined", function(){
			var blockChunk = new BlockChunk();
			blockChunk.addBlock(new BlockCoord(999, 999, 999), new Block());
		})
	})
	...
	describe("#removeBlock", function(){
		it("should throw if out of bounds", function(){
			var blockChunk = new BlockChunk(new BlockCoord(1, 1, 1), 2);
			expect(function(){
				blockChunk.removeBlock(new BlockCoord(0, 0, 0), new Block());
			}).to.throw("out of bounds");
			expect(function(){
				blockChunk.removeBlock(new BlockCoord(3, 3, 3), new Block());
			}).to.throw("out of bounds");
		})

		it("should throw if no existing block found", function(){
			var blockChunk = new BlockChunk(new BlockCoord(1, 1, 1), 2);
			blockChunk.addBlock(new BlockCoord(1, 1, 1), new Block());
			expect(function(){
				blockChunk.removeBlock(new BlockCoord(2, 2, 2))
		...
		it("should remove existing block", function(){
			var blockChunk = new BlockChunk(new BlockCoord(1, 1, 1), 2);
			blockChunk.addBlock(new BlockCoord(1, 1, 1), new Block());
			blockChunk.removeBlock(new BlockCoord(1, 1, 1));
			var block = blockChunk.getBlockOrEmpty(new BlockCoord(1, 1, 1));
	...
	describe("#visitMap", function(){
		it("should visit all blocks", function(){
			var blockChunk = new BlockChunk();
			blockChunk.addBlock(new BlockCoord(1, 2, 3), new Block());
			blockChunk.addBlock(new BlockCoord(-1, -2, -3), new Block());
			blockChunk.addBlock(new BlockCoord(999, 999, 999), new Block());
			blockChunk.addBlock(new BlockCoord(0, 0, 0), new Block());
			var count = 0;
	...
	describe("#shrink", function(){
		it("should reset origin and chunkSize", function(){
			var blockChunk = new BlockChunk();
			blockChunk.addBlock(new BlockCoord(1, 1, 1), new Block());
			blockChunk.addBlock(new BlockCoord(16, 16, 16), new Block());
			blockChunk.shrink();
